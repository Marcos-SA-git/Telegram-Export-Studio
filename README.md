...
Antes de fusionar (o de crear una copia, ver más abajo) la web analiza los exports y, **si el resultado supera los 4 GB**, te recomienda la versión de escritorio, que es más rápida y directa con exports grandes; puedes continuar igualmente.

**Archivos bloqueados por Chrome.** Al escribir cada archivo, Safe Browsing (la protección de Chrome) puede considerarlo sospechoso y no dejar guardarlo en la carpeta. Suele pasar con ejecutables (`.apk`, `.exe`…) o con archivos poco comunes, como algunos stickers animados `.tgs`, y a menudo son falsas alarmas. La operación sigue con el resto, los bloqueados se listan en un desplegable y, al terminar, puedes descargarlos en un **ZIP de rescate** con la misma estructura de carpetas. Copiando su contenido dentro de la carpeta del resultado, cada archivo queda en su sitio y no hay enlaces rotos. Descargarlo es decisión tuya: hazlo si confías en su contenido. La versión de escritorio no pasa por Safe Browsing, así que ahí no ocurre.
...
En la app de escritorio, la fusión y las copias (al compactar, mejorar o desmejorar sobre una copia) se pueden **cancelar** desde la tarjeta de progreso; el resultado a medias se borra si la carpeta de destino estaba vacía, y los exports de origen no se tocan nunca. Lo que modifica el propio export no se puede cancelar, porque pararlo a mitad lo dejaría inconsistente. Para cerrar la app usa el botón de apagar (arriba a la derecha): cerrar la pestaña no la detiene. Si no, se cierra sola tras **una hora sin actividad**, pero nunca mientras haya una operación en curso.
...
- **Web** (`web/app.html`): añade `?debug=1` a la URL. Abre un panel de log con su propio espacio — una columna a la derecha en pantallas anchas, una franja abajo en las estrechas y en el móvil —, de modo que nunca tapa la app; se puede minimizar a su barra de título. Registra: el arranque del motor (tiempos, versión de Pyodide), cada export inspeccionado (mensajes, páginas, tipo de chat — sin nombres de personas), el análisis previo (archivos, tamaño estimado), cada pregunta que se muestra y la opción elegida, el inicio, la duración y el final de cada trabajo (terminado, fallido o cancelado), las páginas leídas y escritas, la copia de media (archivos bloqueados con el error exacto, archivos lentos, velocidad elegida para copiar en paralelo) y el ZIP de rescate (archivos, tamaño, tiempo de la verificación de Chrome), cada aviso y mensaje mostrado al usuario, la traza completa de los errores de Python y de JavaScript (la interfaz solo enseña la última línea), y los errores no capturados. El panel solo sigue el scroll automáticamente mientras estés al final del log, y tiene un botón "Copiar" para volcar todo al portapapeles. El log contiene nombres de carpetas y archivos: revísalo antes de compartirlo. El flag está apagado por defecto y no afecta en nada a quien no lo use — es seguro que quede en el código que se publica a GitHub Pages.
- **Escritorio / AIO** (`telegram_export_studio.py`): botón de icono junto al de apagar el servidor (arriba a la derecha). Al activarlo, cada trabajo escribe en su log (el desplegable "Ver registro completo" de la interfaz): qué operación arranca y con qué parámetros (sin nombres de personas), el timing de cada etapa (inicio, duración y, mientras haya progreso medible, % completado y ETA), el resumen de la copia cuando compactas o mejoras sobre una copia, cada aviso, las cancelaciones (cuándo se piden, cuánto tardó el trabajo y si se borró el resultado a medias), la duración total y, si falla, la traza completa del error. El estado se recuerda entre sesiones (`localStorage`).
